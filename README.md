...
## Install ROS Melodic
Follow instructions on https://wiki.ros.org/melodic/Installation/Ubuntu
...
## Simulation
```
[IMPORTANT: For every new terminal session, make sure you source the catkin workspace's environment 
source $HOME/aerial_robotics_ws/devel/setup.bash

[Launch ROS Gazebo SIM (in new terminal session)]:
roslaunch robowork_minihawk_gazebo minihawk_playpen.launch

[Launch Ardupilot Gazebo SITL (in new terminal session)]:
cd $HOME/aerial_robotics_ws/ardupilot
./Tools/autotest/sim_vehicle.py -v ArduPlane -f gazebo-minihawk --model gazebo-quadplane-tilttri --console  # --map

[Launch Rviz (in new terminal session)]:
...
cd $HOME/aerial_robotics_ws/src/project/src
chmod +x auto_landing_node.py
rosrun project src/auto_landing_node.py
```
